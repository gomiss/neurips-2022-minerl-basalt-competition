...
def de_worker(file_dir, video_dir, save_path, yolo_data_path):
    ...
    TRAIN_PATH = os.path.join(file_dir, 'findcave_train_file_infos.pkl')
    TEST_PATH = os.path.join(file_dir, 'findcave_test_file_infos.pkl')
    ...
    # =============  data convert =============
    video_dir = os.path.join(video_dir, 'MineRLBasaltFindCave-v0')
    ...
    with open(os.path.join(save_path, 'data.yaml'), 'w') as f:
        ...
        f.write('\n')
        f.write('nc: 2\n')
        f.write('names: [\'cave\', \'hole\']')


def train_findcave(
    FILE_DIR='./yolov5/resources/',
    VIDEO_DIR='/data/',
    SAVE_PATH ='./yolov5/data/findcave_data/',
    TRAIN_YOLO_SH_PATH='./yolov5/train_findcave.sh',
    ORIGINAL_YOLO_PT_PATH='./yolov5/runs/train/exp2/weights/best.pt',
    TARGET_YOLO_PT_PATH='./train/findcave_detector.pt'
):
    base_dir = os.path.abspath(__file__)
    base_dir = base_dir[:base_dir.rindex('/')]
    YOLO_DATA_PATH = os.path.join(base_dir, 'data', 'findcave_data')
...
if __name__ == '__main__':
    ...
    base_dir = os.path.abspath(__file__)
    base_dir = base_dir[:base_dir.rindex('/')]
    YOLO_DATA_PATH = os.path.join(base_dir, 'yolov5', 'data', 'findcave_data')

    # 1. 解码yolo数据集 [i7-8700 32G]: 570.76s
    de_worker(FILE_DIR, VIDEO_DIR, SAVE_PATH, YOLO_DATA_PATH)

    # # 2. 训练yolo模型
    f = os.popen('./yolov5/train_findcave.sh')
    print(f.readlines())
    shutil.copyfile(
        './yolov5/runs/train/exp2/weights/best.pt',
        './train/findcave_detector.pt'
    )

    # # 3. 训练vpt模型
    # os.system('./vpt/train_vpt.sh')
