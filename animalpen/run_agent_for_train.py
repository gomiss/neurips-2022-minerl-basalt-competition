...
def main(model, weights, env, n_episodes=3, max_steps=int(6000), show=False):
    ...
    os.system(f"""cp {base_dir}/train/MineRLBasaltFindCave.weights  {base_dir}/train/animal_pen_MineRLBasalt.weights""")
    os.system(f"""cp /data/VPT-models/foundation-model-1x.model  {base_dir}/train/animal_pen_foundation-model-1x.model""")
    os.system(f"""cp {base_dir}/buildVillageHouse/models/flattenareaV5mobilenet.pt  {base_dir}/train/animal_pen_flattenareaV4resnet50.pt""")
