...
class CategoricalActionHead(ActionHead):
    ...
    def __init__(
        ...
        if builtin_linear_layer:
            # self.linear_layer_1 = nn.Linear(input_dim, input_dim)
    ...
    def reset_parameters(self):
        # if self.linear_layer_1 is not None:
        #     init.orthogonal_(self.linear_layer_1.weight, gain=0.01)
        #     init.constant_(self.linear_layer_1.bias, 0.0)
        #     finit.fan_in_linear(self.linear_layer_1, scale=0.01)
    ...
    def forward(self, input_data: torch.Tensor, mask=None) -> Any:
        if self.linear_layer is not None:
            # flat_out = self.linear_layer_1(input_data)
            # flat_out = self.linear_layer(flat_out)
