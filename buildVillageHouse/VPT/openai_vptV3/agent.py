...
from vptaction.actionMap import CameraHierarchicalMappingMoving, CameraHierarchicalMappingV2
...
class MineRLAgent:
    def __init__(self, env, device=None, policy_kwargs=None, pi_head_kwargs=None):
        ...
        # self.action_mapper = CameraHierarchicalMappingV2(n_camera_bins=11)
        self.action_mapper = CameraHierarchicalMappingMoving(n_camera_bins=11)
    ...
    def load_weights(self, path, weights=None):
    ...
    def _env_reward_to_agent(self, reward, to_torch=True):
        if to_torch:
            return th.from_numpy(reward).to(self.device)
        return reward
    ...
    def cal_acc(self, batch_images, batch_actions):
        """
        to calculate the accuracy of current policy on batch data
        ...
        return:
            accuracy:
        """
        # for image, action in zip(batch_images, batch_actions):
        pass
